...
def parse_region(value: str) -> NamedBox:
    try:
    ...
    if not name or any(character not in "abcdefghijklmnopqrstuvwxyz0123456789-_" for character in name):
        raise argparse.ArgumentTypeError(
            "region name must contain only lowercase letters, digits, '-' or '_'"
        )
    return name, parse_box(raw_box)
...
def compare_images(
    ...
    ImageEnhance.Contrast(diff).enhance(3.0).save(output_dir / "diff-enhanced.png")
    Image.blend(reference, current, 0.5).save(output_dir / "overlay.png")
    mask.convert("L").save(output_dir / "mask.png")
    ...
    different = 0
    diff_pixels = (
        diff.get_flattened_data() if hasattr(diff, "get_flattened_data") else diff.getdata()
    )
    mask_pixels = (
        mask.get_flattened_data() if hasattr(mask, "get_flattened_data") else mask.getdata()
    )
    for pixel, included_pixel in zip(diff_pixels, mask_pixels):
        if not included_pixel:
            continue
        included += 1
        if max(pixel) > pixel_threshold:
            different += 1
...
def main() -> None:
    ...
    parser.add_argument(
        "--region", type=parse_region, action="append", default=[],
        help="repeatable name=x,y,width,height region relative to the main crop"
    )
    ...
    region_names = [name for name, _ in args.region]
    ...
    metrics = compare_images(
        reference, current, mask, args.output_dir, args.pixel_threshold
    )
    metrics.update({
    ...
    for name, box in args.region:
        ...
        if x < 0 or y < 0 or x + width > reference.width or y + height > reference.height:
            parser.error(
                f"region {name!r}={box} is outside cropped size {reference.size}"
            )
        region_box = pil_box(box)
        region_metrics = compare_images(
            reference.crop(region_box),
            current.crop(region_box),
            mask.crop(region_box),
            args.output_dir / "regions" / name,
            args.pixel_threshold,
        )
        region_metrics["box"] = list(box)
        metrics["regions"][name] = region_metrics
